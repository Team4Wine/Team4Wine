...
import WineModal from "../modal/wine/WineModal";
...
export default function WineListPage() {
  ...
  useEffect(() => {
    if (!modals[modalId.current]?.isVisible) {
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.addContainer}>
        ...
        <div className={styles.addObject} onClick={() => setIsOpenModal(true)}>
          <HiPlusCircle className={styles.add}/>
        </div>
      </div>
      {IsOpenModal && <WineModal isClick={true} />}
    </div>
  );
}
